...
- `telegram-archiver channels` runs the configured channel source.
...
- `telegram-archiver all` runs bot and Saved Messages under one supervisor.
- `docker compose --profile channels up -d channels-archiver` starts the optional channel archiver service in Docker.
